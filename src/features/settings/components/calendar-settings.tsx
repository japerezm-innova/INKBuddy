...
import { Calendar, Copy, Check, RefreshCw, Apple, Chrome } from 'lucide-react'
...
export function CalendarSettings({ initialToken }: Props) {
  ...
  function handleRegenerate() {
    if (
      !window.confirm(
        '¿Regenerar el token? La URL anterior dejara de funcionar.'
      )
    )
      return
  ...
  return (
    <GlassCard className="p-5 space-y-5">
      ...
      <div className="space-y-2">
        <p className="text-sm font-medium text-ink-dark/70">
          URL de suscripcion
        </p>
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-start gap-3 p-3 rounded-2xl bg-white/30 border border-white/40">
            <Chrome className="h-5 w-5 text-ink-dark/60 shrink-0 mt-0.5" />
            <div>
              <p className="text-sm font-medium text-ink-dark">
                Google Calendar
              </p>
              <p className="text-xs text-ink-dark/50 mt-0.5">
                Abre Google Calendar → Otros calendarios (+) → Desde URL →
